...
class Git extends EventEmitter {
   ...
   * @param  {String}  options.initialBranch - If specified, this branch will be given to the `git init --initial-branch=options.initialBranch` command like so - NOTE Do not set this option if your git version on your host is <2.28, the option is not supported
  */
  ...
  create(repo, callback) {
      ...
      function next (err) {
          ...
          if (self.repoTemplatePath) {
              args.push('--template=' + self.repoTemplatePath)
          }

          if (self.initialBranch) {
              // NOTE This doesn't work in git versions previous to git 2.28,
              // setting it on prior git versions will cause this operation to
              // fail
              args.push('--initial-branch=' + self.initialBranch)
          }
